...
if options["m"] != "" and options["p"] != "" and options["n"] != "":
    ...
    if proc_id == root_process:
        ...
        print(
            "    DFT supercell grid size : "
            + str(mat_mod.mat_properties_dict["supercell_dim"])
            + "\n"
        )
        ...
        print(
            "    - d log F_med / d log q : " + str(phys_mod.physics_parameters["Fmed_power"]) + "\n"
        )
        ...
        print(
            "    Energy bin width : "
            + str(num_mod.numerics_parameters["energy_bin_width"])
            + " eV\n"
        )
    ...
    yaml_path = os.path.join(os.path.split(material_input)[0], "phonopy.yaml")
    ...
    for job in range(len(job_list_recv)):
        if job_list_recv[job, 0] != -1 and job_list_recv[job, 1] != -1:
            ...
            delta = (
                2 * phys_mod.physics_parameters["power_V"]
                - 2 * phys_mod.physics_parameters["Fmed_power"]
            )
            ...
            k_red_list = mesh.generate_k_red_mesh_from_q_XYZ_mesh(
                q_XYZ_list, phonopy_params["recip_red_to_XYZ"]
            )
            ...
            if "special_model" in phys_mod.physics_parameters.keys():
                ...
                [diff_rate, binned_rate, total_rate] = physics.calc_diff_rates_general(
                    mass,
                    q_XYZ_list,
                    G_XYZ_list,
                    jacob_list,
    ...
    # It's worth putting this here anyway because there's a barrier in the
    # parallel HDF5 writing routines.
    comm.Barrier()
    if proc_id == root_process:
        print("Done computing rate on all ranks. Writing to file.")
    ...
    out_filename = os.path.join(
        ...
        + "_"
        + num_input_mod_name
        + num_mod.io_parameters["output_filename_extra"]
        + ".hdf5",
    )

    hdf5_output.hdf5_write_output_parallel(
        out_filename,
        num_mod.numerics_parameters,
        phys_mod.physics_parameters,
        phys_mod.dm_properties_dict,
